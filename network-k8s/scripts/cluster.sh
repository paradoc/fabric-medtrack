...
function pull_docker_images() {
  ...
  # NOTE: Modified for Apple M2 Chip
  $CONTAINER_CLI pull --platform aarch64 ${CONTAINER_NAMESPACE} ${FABRIC_CONTAINER_REGISTRY}/fabric-ca:$FABRIC_CA_VERSION
  $CONTAINER_CLI pull --platform aarch64 ${CONTAINER_NAMESPACE} ${FABRIC_CONTAINER_REGISTRY}/fabric-orderer:$FABRIC_VERSION
  $CONTAINER_CLI pull --platform aarch64  ${CONTAINER_NAMESPACE} ${FABRIC_PEER_IMAGE}
  $CONTAINER_CLI pull --platform aarch64 ${CONTAINER_NAMESPACE} couchdb:3.2.1

  $CONTAINER_CLI pull --platform aarch64 ${CONTAINER_NAMESPACE} ghcr.io/hyperledger/fabric-rest-sample:latest
  $CONTAINER_CLI pull --platform aarch64 ${CONTAINER_NAMESPACE} redis:6.2.5
